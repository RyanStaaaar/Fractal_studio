...
class Plan :
    def __init__(self, size) :
        self.canvas = Image.new("RGB", (size,size), color = (255,255,255))
        self.centre = (size//2, size//2) # le centre est plus ou moins le vrai centre
        self.figure = ImageDraw.Draw(self.canvas)
    def draw_point(self,coordinates, color=0) :
        self.figure.point((coordinates[0] + self.centre[0], coordinates[1] + self.centre[1]), fill=color)
    def display(self) :
